...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if(btn) {
        btn.addEventListener('click', function() {
            ...
            .then(response => {
                if (response.ok) {
                    appendLogMessage('Supply ON');                 

                } else {
                    appendLogMessage('Supply ON Failed');
                }
            })
            .catch(error => {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (resetButton && recalButton) {
        resetButton.addEventListener('click', function () {
            slider.value = 0;
            appendLogMessage('Resetdone');
        });

        recalButton.addEventListener('click', function () {
            slider.disabled=false;
            slider.value = 0;
            appendLogMessage('Recalibrated');
        });
    }
    ...
    function updateBlynkSliderValue(newValue) {
        ...
        fetch(url)
            ...
            .then(data => {
                appendLogMessage(`variac position changed note the observations from table-1.`);
            })
            .catch(error => {
                appendLogMessage('Error Updating Variac Position', error);
            });
    }
});
                    ...
                    if (response.ok) {
                        ...
                        setTimeout(function () {
                            ...
                            fetch(resetUrl)
                                .then(response => {
                                    if (response.ok) {
                                        appendLogMessage('Variac Updated');
                                    } else {
                                        console.log('Error Updating Variac');
                                    }
                                })
                        }, 200);
                    } else {
...
function appendLogMessage(message) {
    const activityLogCard = document.getElementById('card');

    const logMessageParagraph = document.createElement('p');
    logMessageParagraph.textContent = message;
    
    logMessageParagraph.style.color='black';
    logMessageParagraph.style.marginTop='10px';
    activityLogCard.appendChild(logMessageParagraph);    
